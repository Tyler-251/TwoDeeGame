...
pub fn crab_init(mut commands: Commands) {
    commands.spawn((
        Crab {
            speed: 10.0,
            legs: 8,
...
pub fn gimme_crab(query: Query<&Crab>) {
    println!("Crabs:");
...
pub fn gimme_named_crabs(query: Query<(&Crab, &CrabName)>) {
...
pub fn gimme_nameless_crab(query: Query<&Crab, Without<CrabName>>) {
...
pub fn gimme_special_crabs(query: Query<(&Crab, &CrabName), (With<CrabSize>, With<CrabColor>)>) {
...
//mutable query
pub fn change_all_crab_names(mut query: Query<&mut CrabName>) {
